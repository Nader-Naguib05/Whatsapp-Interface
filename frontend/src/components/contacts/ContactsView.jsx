...
const ContactModal = ({
    ...
    const phoneError =
        touched && !form.phone.trim() ? "رقم الهاتف مطلوب" : undefined;
                ...
                <div className="flex-1 text-xs text-slate-700 space-y-1">
                    ...
                    <div className="space-y-0.5">
                        ...
                        <div className="text-[11px] text-slate-500 mt-0.5">
                            الرقم بعد التطبيع متطابق. اختر جهة اتصال للحفاظ
                            عليها أو تجاهل التحذير.
                        </div>
                    </div>
                </div>
...
const ContactsView = ({ onContactsChange }) => {
    ...
    const findDuplicateForContact = (contact, list) => {
        ...
        const other = list.find(
            (c) =>
                c._id !== contact._id && normalizePhone(c.phone || "") === norm
        );
        ...
        try {
            ...
            setModalError(
                err?.response?.data?.error ||
                    "فشل حفظ جهة الاتصال. حاول مرة أخرى."
            );
        } finally {
    ...
    const handleMerge = async (keep, remove) => {
        ...
        try {
            ...
            if (favorites.includes(removeInState._id)) {
                const nextFav = favorites.filter(
                    (id) => id !== removeInState._id
                );
                persistFavorites(nextFav);
            }
